...
BEGIN
  ...
  IF v_it_course_id IS NOT NULL THEN
    ...
    INSERT INTO public.certification_questions (certification_id, question, options, correct_index, explanation) VALUES
    (v_it_course_id, '¿Qué estructura de datos opera bajo el principio LIFO (Last In, First Out)?', ARRAY['Cola (Queue)', 'Pila (Stack)', 'Lista enlazada circular', 'Árbol binario de búsqueda'], 1, 'El último elemento en ingresar a la pila es el primero en ser extraído (LIFO).'),
    (v_it_course_id, '¿Cuál es la complejidad temporal de una búsqueda binaria sobre un arreglo ordenado de N elementos?', ARRAY['O(N)', 'O(log N)', 'O(N^2)', 'O(1)'], 1, 'La búsqueda binaria divide el espacio de búsqueda a la mitad en cada paso, logrando O(log N).'),
    (v_it_course_id, '¿Qué tipo de dato booleano representa la expresión (true && false) || (!false)?', ARRAY['false', 'true', 'null', 'undefined'], 1, '(true && false) es false; (!false) es true; false || true resulta en true.'),
    (v_it_course_id, '¿Cuál es el propósito fundamental de una función pura en programación?', ARRAY['Modificar variables globales del sistema', 'Devolver siempre el mismo resultado para los mismos argumentos sin causar efectos secundarios externos', 'Imprimir texto en la consola de depuración', 'Conectarse a una base de datos externa'], 1, 'Las funciones puras son deterministas y no alteran el estado fuera de su propio ámbito.'),
    (v_it_course_id, '¿Qué problema resuelve el uso de Git y sistemas de control de versiones?', ARRAY['Aumentar la velocidad de la memoria RAM', 'Rastrear el historial de cambios en el código, facilitar la colaboración en equipo y permitir ramificaciones (branches)', 'Compilar código JavaScript en tiempo real', 'Eliminar automáticamente bugs del código'], 1, 'Git permite gestionar versiones, colaborar concurrentemente y revertir cambios cuando sea necesario.');
  END IF;
